...
<script>
    $(document).ready(function () {
        ...
        $("#otpForm").submit(function (event) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        alert("OTP Verified!");
                        $("input[name='otp']").val(""); // Clear the OTP field
                        $("#otpModal").modal("hide");
                        submitEvaluationForm(); // Proceed to submit evaluation
                    } else {
        ...
        // Ensure modal can be closed manually with X button
        $(".close").on("click", function () {
            $("#otpModal").modal("hide");
        });

        // Clear OTP field when modal is closed
        ...
        function submitEvaluationForm() {
            let formData = new FormData($("#evaluationSubmit")[0]); // Get form data
            formData.append("email", "{{ session('otp_email') }}"); // Ensure email is included

            console.log("Submitting Evaluation Form Data:", [...formData.entries()]); // Debugging form data
