...
class SuperAdminController extends Controller
{
    ...
    // Import Carbon at the top of your controller
    public function userListView()
    {
        ...
        $users = SuperAddUser::where('probation_date', '<=', $currentDate)->get();
    ...
    public function updateStatus(Request $request, $employeeId)
    {
       ...
       public function updateProbationDate(Request $request, $employeeId)
    {
        $user = SuperAddUser::where('employee_id', $employeeId)->first();

        if (!$user) {
            return response()->json(['message' => 'User not found.'], 404);
        }

        $newProbationDate = $request->probation_date;

        if (!$newProbationDate) {
            return response()->json(['message' => 'Probation date is required.'], 400);
        }


        $user->probation_date = $newProbationDate;
        $user->save();

        $appraisal = \App\Models\ApprisalTable::latest()->first();
        ...
        if ($appraisal && $appraisal->company_percentage && $appraisal->financial_year) {
            ...
            $parsedProbationDate = Carbon::parse($newProbationDate);

            if ($parsedProbationDate->between($startDate, $endDate) && $parsedProbationDate->lte(Carbon::today())) {
        ...
        return response()->json([
            'message' => 'Probation date updated successfully.',
        ]);
    }
}
