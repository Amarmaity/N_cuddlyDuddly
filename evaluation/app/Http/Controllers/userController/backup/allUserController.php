...
class allUserController extends Controller
{
    ...
    {
        return view('delostyleUsers/admin-dashboard');  // Replace with actual view for admin dashboard
    }
    ...
    public function searchUser(Request $request)
    {
        ...
        if (!empty($employeeId)) {
            // Search by Employee ID (returns one record)
            $query->where('employee_id', $employeeId);
        } elseif (!empty($name)) {
            // Search by Name (returns multiple records)
            $query->whereRaw("CONCAT(fname, ' ', lname) LIKE ?", ["%$name%"]);
        }

        $users = $query->get(); // Get multiple records instead of first()
    ...
    public function hr()
    {
        return view('delostyleUsers/hr-dashboard');  // Replace with actual view for HR dashboard
    }
    ...
    public function hrReviewStore(Request $request)
    {
        ...
        // Insert using create() (automatically updates timestamps)
    ...
    {
        return view('delostyleUsers/users-dashboard');  // Replace with actual view for user dashboard
    }

    public function manager()
    {
        return view('delostyleUsers/manager-dashboard');  // Replace with actual view for manager dashboard
    }
    ...
    public function managerReviewStore(Request $request)
    {
        ...
        // Insert data using Eloquent create()
    ...
    {
        return view('delostyleUsers/client-dashboard');  // Replace with actual view for client dashboard
    }
    ...
    public function clientReviewStore(Request $request)
    {
        ...
        try {
            ...
            // Log user_roles for debugging
            Log::info('User Roles:', ['roles' => $user->user_roles ?? 'No user found']);

            // Ensure the user exists and decode user_roles
            if ($user) {
                $roles = json_decode($user->user_roles, true); // Decode JSON string to array

                if (is_array($roles) && in_array('client', $roles)) {
                    // Save the review in ClientReviewTable
                    ClientReviewTable::create($validatedData);
                    return response()->json(['message' => 'Review submitted successfully!'], 200);
                }
            }

            // If user is not a client, return unauthorized response
            return response()->json(['error' => 'You are not authorized to submit this review.'], 403);
        } catch (\Exception $e) {
            Log::error('Error in clientReviewStore: ' . $e->getMessage());
    ...
    public function getHrReviewsList(Request $request)
{
    ...
    public function showEvaluationDetails($employee_id){
        ...
        $eval = evaluationTable::where('emp_id', $employee_id)->get();
